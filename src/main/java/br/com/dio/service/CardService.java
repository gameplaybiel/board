...
public class CardService {
    ...
    public void moveToNextColumn(final Long cardId, List<BoardColumnInfoDTO> boardColumnsInfo) throws SQLException{
        try {
            ...
            if(dto.blocked()){
                var message = "O card de %s está bloqueado, é necessário desbloquea-lo para mover".formatted(cardId);
    ...
    public void block(final Long id, final String reason,
        ...
        try {
            ...
            if(dto.blocked()){
                var message = "O card de %s já está bloqueado".formatted(id);
                throw new CardBlockedException(message);
            }
            var currentColumn = boardColumnsInfo.stream()
                    .filter(bc -> bc.id().equals(dto.columnId()))
    ...
    public void unblock(final Long id, final String reason) throws SQLException {
        try {
            ...
            if(!dto.blocked()){
                var message = "O card de %s já está desbloqueado".formatted(id);
                throw new CardBlockedException(message);
            }
            var blockDAO = new BlockDAO(connection);
